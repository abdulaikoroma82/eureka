...
def test_valid_but_unsupported_xpath_is_passed_through():
    """The XPath union operator is valid but outside the local grammar."""
    qn = Questionnaire(
        settings=FormSettings(form_title="T", form_id="t"),
        questions=[Question(
            name="place", xlsform_type="select_one places", label="Place",
            choice_filter="item[@type='urban'] | item[@type='rural']")])
    findings = ExpressionValidator().validate(qn)
    assert len(findings) == 1
    assert findings[0].level == "info"
    assert findings[0].confidence == "unsupported"
    assert "not fully check" in findings[0].message
